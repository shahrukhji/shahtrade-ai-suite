...
import StrategyConfigSheet from '@/components/Strategies/StrategyConfigSheet';
import ActiveStrategies from '@/components/Strategies/ActiveStrategies';
import AIStrategySuggestion from '@/components/Strategies/AIStrategySuggestion';
import { type Strategy, ALL_STRATEGIES } from '@/data/strategies';

const instruments = ['NIFTY', 'BANKNIFTY', 'FINNIFTY', 'MIDCPNIFTY', 'SENSEX'];
const expiries = ['16 Jan', '23 Jan', '30 Jan', '27 Feb', '27 Mar', '26 Jun'];

const tabs = ['Chain', 'Strategies', 'AI Suggest', 'Active'];
  ...
  return (
    <div className="space-y-4">
      ...
      {activeTab === 'Strategies' && (
        <StrategySelector onSelectStrategy={handleSelectStrategy} />
      )}
      ...
      {activeTab === 'AI Suggest' && (
        <AIStrategySuggestion
          ...
          spotPrice={spotPrice}
          onApplyStrategy={(id) => {
            const s = ALL_STRATEGIES.find((x: Strategy) => x.id === id);
            if (s) handleSelectStrategy(s);
          }}
        />
      )}

      {activeTab === 'Active' && <ActiveStrategies />}

      <Disclaimer />

      <StrategyConfigSheet strategy={selectedStrategy} isOpen={showConfig} onClose={() => setShowConfig(false)} />
    </div>
  );
};
